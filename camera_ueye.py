"""DEPRECATED - IDS uEye driver, replaced by the Spinnaker ``camera.py``.

Preserved verbatim from the pre-migration ``camera.py`` (commit 297b44c) for
side-by-side reference and diffing against the new Spinnaker driver. NOT
imported by the running app -- the app imports ``camera``. Requires
``pyueye``, so this module becomes import-broken if ``pyueye`` is later
uninstalled. Reference / archival only.
"""
# camera.py
